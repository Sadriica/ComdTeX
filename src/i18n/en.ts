...
export const en: T = {
  toolbar: {
    ...
    taskList: "Task list", link: "Link", codeBlock: "Code block",
    ...
    table: "Table", matAuto: "Auto matrix", matFixed: "Fixed matrix", matLiteral: "Literal matrix",
    more: "More",
    math: "Math",
    ...
    lbl_separator: "—  Separator", lbl_list: "•  List",
  ...
  settings: {
    ...
    sectionPreview: "Preview", sectionGeneral: "General",
    ...
    hints: {
      ...
      touchpadGestures: "Ctrl+wheel and pinch zoom, and a swipe from the screen edge to change tab. Turn it off if your touchpad triggers them by accident.",
      fontSize: "Size of the text you write, in pixels. Ctrl+wheel over the editor changes it too.",
      autoSaveMs: "How long ComdTeX waits after you stop typing before saving. Shorter is safer, longer keeps a slow disk quiet.",
      wordGoal: "A daily target for the active document, shown as progress in the status bar. Zero turns the goal off.",
      vimMode: "Vim keys and modes inside the editor. Everything else in the app keeps working as usual.",
      typewriterMode: "Keeps the line you are writing vertically centred, so your eyes stay in one place.",
      wordWrap: "Long lines fold to the pane width instead of running off to the right.",
      minimapEnabled: "The small map of the document on the right edge of the editor.",
      previewFontSize: "Size of the rendered text on the preview side, independent of the editor.",
      previewVisible: "Shows the rendered pane beside the editor. Hiding it makes typing lighter in very long documents.",
      syncScroll: "The preview follows the editor as you move, so both sides show the same place.",
      mathPreview: "Shows the rendered formula when the cursor sits on display math, without leaving the editor.",
      previewTheme: "Colours for the preview. Keep it light while the editor is dark if you want to see the page as it will print.",
      dailyNotesEnabled: "Turns on Ctrl+Shift+D, which opens today's note and creates it the first time.",
      dailyNotesFolder: "Folder inside the vault where those notes are created. It is made if it does not exist.",
      aiProviderId: "Who answers. Pick OpenAI-compatible for any other service, or for a model running on this machine.",
      aiBaseUrl: "The address of that service, for example http://localhost:11434/v1 for Ollama on this machine.",
    },
    sections: {
      general: "General",
      editor: "Editor",
  ...
  palette: {
    ...
    viewProperties: "View properties", viewGraph: "View graph",
    viewEquations: "View equations", viewStats: "Vault statistics",
  ...
  aiSettings: {
    ...
    testing: "Testing…",
    testOk: "Connected. The provider answered.",
    testIncomplete: "Something is still empty: fill in the fields above first.",
    testBadUrl: "That address is not allowed: use https:// or a local address.",
    testUnauthorized: "The provider refused the key. Check that it is current and belongs to this provider.",
    testNoModel: "The provider answered, but does not have that model. Check the exact model name.",
    testUnreachable: "No answer at all. Check the address, and that the service is running if it is local.",
    testFailed: "The provider answered with an error.",
    hostNote: "Any provider reachable over https works, and so does one running on this machine. Requests never pass through Witara.",
  },
  ...
  cloudSync: {
    ...
    settings: {
      ...
      posture: "How this vault travels",
      postureGitShared: "Versioned and shared. Every change has a record, and a copy lives outside this machine.",
      postureGitLocal: "Versioned, but the history exists only on this machine. If the disk goes, it goes with it.",
      postureGitInCloud: "This vault is a git repository inside a synced cloud folder. The cloud client and git both write to the same internal files, which corrupts the history sooner or later. Move the vault out of the cloud folder and share it with a remote instead.",
      postureCloudOnly: "Copies travel, nothing is versioned. You cannot see what changed, and two machines editing at once leaves a second file instead of asking you.",
      postureLocalOnly: "Nothing leaves this disk.",
      stepMoveOutOfCloud: "Move the vault out of the cloud folder",
      stepAddRemote: "Give it somewhere to push to",
      stepStartGit: "Start keeping history with git",
      openGit: "Open the Git panel",
      safetyNet: "Safety net for vaults kept in a cloud folder. It never moves or deletes anything on its own: it only points out the conflict copies these services leave behind.",
    },
  },
}
